...
$(function () {
 ...
 console.log('Google Maps JSON response:', data);
    ...
    function latLngGetter(data) {
        ...
        const latLngLoc = { latitude, longitude };
    ...
    function getCentOfTwoLocs(locationsArr) {
        ...
        const lat1 = locationsArr[0].latitude;
        const lat2 = locationsArr[1].latitude;

        // Obtains Longitude for Locations 1 & 2
        const lng1 = locationsArr[0].longitude;
        const lng2 = locationsArr[1].longitude;
    ...
    function centerMap(coordinatesObj) {
        const geoCenLat = coordinatesObj.latCen;
        const geoCenLng = coordinatesObj.lngCen;
...
// This should almost work somehow:
    ...
    function submitHandler() {
        ...
        $('form').submit(function (e) {
            ...
            // Makes 1st call to get and display job data based on user
            // selected location and returns GeoCode for 1st Location
            const city1jobs = getAdzunaJobSearch(userInputCountry, userInputCity, jobCategory)
            ...
            // Makes 2nd call to get and display job data based on user
            // selected location and returns GeoCode for 2nd Location
            const city2jobs = getAdzunaJobSearch(userInputCountry2, userInputCity2, jobCategory)
            // Displays Job data (Salary Mean and Total Available Job Count)
            .then((data) => {
                displayAdzunaJobSearch(data);
            ...
            // Brings both GeoCoded locations together to find
            // center and re-centers map on new coordinates
            const locationPromises = [city1jobs, city2jobs];
            Promise.all(locationPromises)
            .catch(err => console.error('All location Promises did not work, the error is:', err))
            .then((data) => {
console.log('This is the Lat/Lng Array of Objects:', data);
                return data;
            })
